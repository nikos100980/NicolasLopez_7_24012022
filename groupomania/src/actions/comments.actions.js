...
export const getComments = (messageId) => {
    return (dispatch) => {
        return axios
            ({
                method: "get",
                url: `${process.env.REACT_APP_API_URL}api/messages/`+ messageId +"/comments/",
                withCredentials: true,
                
            })
            .then((comment) => {
                dispatch({ type: GET_COMMENTS, payload: { messageId, comments: comment.data } })
            })
...
export const addComment = (messageId, userId, content) => {
    ...
    return (dispatch) => {
        return axios
            ({
                method: "post",
                url: `${process.env.REACT_APP_API_URL}api/messages/comments/` ,
                withCredentials: true,
                data: {userId, content,messageId},
            })
            .then((res) => {
                console.log(res);
                dispatch({ type: ADD_COMMENTS, payload: { messageId, comments: res.data } });
            })
            .catch((err) => console.log(err))
    }
}
